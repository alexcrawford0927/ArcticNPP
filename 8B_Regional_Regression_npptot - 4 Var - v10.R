...
dfr1 <- data.frame(month=rep(ms,length(regions)),reg=reglist,
                  ...
                  int=0,opendays=0,oisst=0,hwf=0,uWper=0,
                  int_p=0,opendays_p=0,oisst_p=0,hwf_p=0,uWper_p=0,
                  int_e=0,opendays_e=0,oisst_e=0,hwf_e=0,uWper_e=0)
...
for(m in ms){
  ...
  for(reg in regions){
    # Create Model
    lm1 <- lm(npptot~opendays+oisst+hwf+uWper, data.frame(scale(df[df$Reg == reg,])) )
    
    # Assign Coefficients
    dfr1[dfr1$month == m & dfr1$reg == reg,c("int","opendays","oisst","hwf","uWper")] <- lm1$coefficients
    
    # Assign p-values & errors
    pvals <- summary(lm1)$coefficients[,4]
    pvals <- c(pvals,rep(NA,length(lm1$coefficients)-length(pvals)))
    dfr1[dfr1$month == m & dfr1$reg == reg,c("int_p","opendays_p","oisst_p","hwf_p","uWper_p")] <- pvals
    
    errors <- summary(lm1)$coefficients[,2]
    errors <- c(errors,rep(NA,length(lm1$coefficients)-length(errors)))
    dfr1[dfr1$month == m & dfr1$reg == reg,c("int_e","opendays_e","oisst_e","hwf_e","uWper_e")] <- errors
...
write.csv(dfr1,paste0("/Volumes/Prospero/Arrigo/Regional",ct,"/ArrigoData_Regional",v,"_4VarModels_opendays_sst_hwf_uWper.csv"),row.names=F)
...
dfr2 <- data.frame(month=rep(ms,length(regions)),reg=reglist,
                   r2=0,BIC=0,AIC=0,bp_p=NA,sw_p=NA,max_vif=NA,
                   int=NA,opendays=NA,oisst=NA,hwf=NA,uWper=NA,
                   int_p=NA,opendays_p=NA,oisst_p=NA,hwf_p=NA,uWper_p=NA,
                   int_e=NA,opendays_e=NA,oisst_e=NA,hwf_e=NA,uWper_e=NA)

indices = c(12,9,14,15,16)
...
write.csv(dfr2,paste0("/Volumes/Prospero/Arrigo/Regional",ct,"/ArrigoData_Regional",v,"_BestBICModels_4Var_opendays_sst_hwf_uWper.csv"),row.names=F)

######################################################################################
# VIFs #
# 
# # ANNUAL
# vif(lm(npptot~opendays+oisst+hwf,dfa[dfa$Reg == 8,]))
# vif(lm(npptot~opendays+oisst,dfa[dfa$Reg == 10,]))
# vif(lm(npptot~opendays+oisst,dfa[dfa$Reg == 11,]))
# vif(lm(npptot~opendays+oisst+hwf,dfa[dfa$Reg == 17,]))
# vif(lm(npptot~opendays+oisst,dfa[dfa$Reg == 13,]))
# 
# # JUNE
# vif(lm(npptot~opendays+oisst,dfm[dfm$Reg == 9 & dfm$Month == 6,]))
# vif(lm(npptot~opendays+oisst,dfm[dfm$Reg == 17 & dfm$Month == 6,]))
# vif(lm(npptot~opendays+hwf,dfm[dfm$Reg == 18 & dfm$Month == 6,]))
# vif(lm(npptot~opendays+oisst+uWper,dfm[dfm$Reg == 13 & dfm$Month == 6,]))
# 
# # JULY
# vif(lm(npptot~opendays+oisst,dfm[dfm$Reg == 8 & dfm$Month == 7,]))
# vif(lm(npptot~opendays+oisst,dfm[dfm$Reg == 9 & dfm$Month == 7,]))
# vif(lm(npptot~opendays+hwf,dfm[dfm$Reg == 10 & dfm$Month == 7,]))
# vif(lm(npptot~opendays+oisst,dfm[dfm$Reg == 11 & dfm$Month == 7,]))
# vif(lm(npptot~opendays+oisst+hwf,dfm[dfm$Reg == 17 & dfm$Month == 7,]))
# vif(lm(npptot~opendays+oisst+uWper,dfm[dfm$Reg == 18 & dfm$Month == 7,]))
# vif(lm(npptot~opendays+oisst,dfm[dfm$Reg == 13 & dfm$Month == 7,]))
# 
# # AUGUST
# vif(lm(npptot~opendays+oisst,dfm[dfm$Reg == 10 & dfm$Month == 8,]))
# vif(lm(npptot~opendays+oisst+hwf,dfm[dfm$Reg == 17 & dfm$Month == 8,]))
# vif(lm(npptot~opendays+oisst+uWper,dfm[dfm$Reg == 13 & dfm$Month == 8,]))
# 
# # SEPTEMBER
# vif(lm(npptot~opendays+oisst,dfm[dfm$Reg == 8 & dfm$Month == 9,]))
# vif(lm(npptot~opendays+hwf,dfm[dfm$Reg == 9 & dfm$Month == 9,]))
# vif(lm(npptot~opendays+oisst+uWper,dfm[dfm$Reg == 10 & dfm$Month == 9,]))
# vif(lm(npptot~opendays+uWper,dfm[dfm$Reg == 11 & dfm$Month == 9,]))
# vif(lm(npptot~opendays+oisst,dfm[dfm$Reg == 17 & dfm$Month == 9,]))
# vif(lm(npptot~opendays+oisst,dfm[dfm$Reg == 18 & dfm$Month == 9,]))
# 
