...
for m in mos:
    ...
    for y in years:
        ...
        ### LOAD FILES AND CALCULATE MONTHLY SUM ###
        print(" -- Summing "+Y)
        arrMlist = []
        ...
        sumMlist.append( np.apply_along_axis(np.nansum,0,arrM) )
    
    sumM = np.array(sumMlist)
    
    ### Write new netcdf file ###
    fout = "prod_arctic_66N_Month"+M+"_"+YY+".nc"
...
### Write new netcdf file ###
fout = "prod_arctic_66N_Monthly_Trend_"+YY+".nc"
...
ncf2.description = 'Trend ('+YY+') in Monthly Net Primary Productivity from the Arrigo Model'
ncf2.source = 'netCDF4 python module'
tNC.units = 'months'
latNC.units = 'degrees north'
lonNC.units = 'degrees east'      
bNC.units = 'Trend (per yr) in Monthly Sum of mg C/m^2'
aNC.units = 'Intercept of in Monthly Sum of mg C/m^2'
